...
class LSBSteg():
    ...
    def hideText(self, txt):
        l = len(txt)
        # print l
        binl = self.binValue(l,32) #Length coded on 2 bytes so the text size can be up to 65536 bytes long
        # print binl
    ...
    def unhideText(self):
        ls = self.readBits(32) #Read the text size in bytes
        l = int(ls,2)
        i = 0
        unhideTxt = ""
...
#file=open(str1,"rb")
#str=file.read()
#carrier = cv.LoadImage("7z.png")
#steg = LSBSteg(carrier)
#steg.hideText(str)
#steg.saveImage("res.png") #Image that contain datas

im = cv.LoadImage("7z.png")
steg = LSBSteg(im)
...
#while(1):
   # pass
#Hide
# imagetohide = cv.LoadImage("22.png")
# carrier = cv.LoadImage("123.png")
# steg = LSBSteg(carrier)
